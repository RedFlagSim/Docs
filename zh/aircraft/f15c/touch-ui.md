...
| 序号 | 控件 | 简要功能 |
| --- | --- | --- |
| 1 | [功能面板](#item-1) | 打开 MAIN、NAV、HUD 和 MISC 功能面板 |
| 2 | [模式切换](#item-2) | 切换 NAV 导航模式与 AA 空空模式 |
| 4 | [主菜单](#item-4) | 打开飞机设置、偏好设置和任务选项 |
| 5 | [输入指示器](#item-5) | 显示操纵输入与配平位置 |
| 6 | [起落架开关](#item-6) | 控制起落架收放 |
| 7 | [襟翼开关](#item-7) | 控制襟翼收放 |
| 8 | [油门](#item-8) | 调整推力；切换至单发动机油门控制 |
| 9 | [油门模式开关](#item-9) | 选择手动、IAS 跟随或在速迎角油门模式 |
| 10 | [减速板开关](#item-10) | 控制减速板展开、停止或收回 |
| 11 | [显示 MFD](#item-11) | 按住时将座舱视角对准 VSD 或 MPCD |
| 12 | [显示战术地图](#item-12) | 显示飞行中的战术地图 |
| 13 | [paddle 开关](#item-13) | 按住时暂时断开自动驾驶 |
| 14 | [浮动 MFD](#item-14) | 显示最多三个浮动 MFD，无需切换视角 |
| 15 | [舵](#item-15) | 控制方向舵、前轮转向和机轮刹车 |
| 16 | [padlock](#item-16) | 将视角锁定到附近实体 |
| 17 | [相机切换菜单](#item-17) | 切换相机视角或打开相机菜单 |
| 18 | [Cursor](#item-18) | 移动 VSD/SIT 游标并指定目标 |
| 19 | [castle 开关](#item-19) | 切换当前传感器（SOI）并管理显示页面 |
| 20 | [状态栏](#item-20) | 显示帧率、风、飞机重量、时间和自动驾驶状态 |
| 21 | [chaff/flare 释放](#item-21) | 释放箔条和热焰弹 |
| 22 | [主武器开关](#item-22) | 启用武器发射 |
...
快速进入方法：在屏幕空白处连续快速点击 4 次。也可以通过主菜单进入。

![UI 元素编辑提示](/assets/F-15C/TouchUI/edit_mode.png)

![UI 编辑模式的 MFD 控制](/assets/F-15C/TouchUI/edit_mode_mfd.png)
...
- `Tac Map`：打开战术地图
- `Tactical`：战术菜单，用于与友军通讯并向僚机下达命令
- `Freeze / Resume`：动态暂停系统；暂停飞行动力学时，其他组件仍可继续工作
- `Playback`：进入回放模式
- `Restart`：重启当前任务
- `Calibrate Gyro`：校准陀螺仪
- `UI Editor`：进入 UI 元素编辑模式
- `Quit`：退出当前任务
...
驾驶 F-15C 时，即使你没有手动配平，也可能看到配平圆圈自动移动。这是因为 PTC 会自动调整配平。
...
<img class="touch-ui-control-thumb" src="/assets/F-15C/TouchUI/controls/14-floating-mfd.png" alt="浮动 MFD">

以浮动窗口的形式在飞行画面上显示 MFD 信息，无需切换当前相机视角即可持续查看显示内容。

最多可同时显示 3 个浮动 MFD，并且可以调整大小。在 iPad 等大屏幕设备上，可以考虑同时显示多个 MFD，方便查看不同信息。

浮动 MFD 持续可见，需要持续渲染，因此同时显示多个 MFD 会增加渲染负担。较老或性能较弱的设备不建议同时开启多个浮动 MFD。
...
- **点击：** 在雷达的 `STBY` 与 `LRS` 模式之间切换。
- **向上滑动：** 按照 `SS` → `VS` → `BST` → `LR BST` → `SS` 的顺序循环切换自动截获模式。
- **向上滑动并保持：** 激活 `FLOOD` 模式。

自动截获模式：

1. **`SS` — Supersearch（超级搜索）：** 以六条扫描线搜索 HUD 视场内 20° × 20° 的区域，并自动锁定 500 ft 至 10 NM 内首先探测到的目标。
2. **`VS` — Vertical Scan（垂直扫描）：** 扫描机身基准线上方 5° 至 55°、方位宽度 7.5° 的区域，可在 500 ft 至 10 NM 内自动截获目标。
3. **`BST` — Boresight（轴线瞄准）：** 沿雷达瞄准轴线搜索；将目标置于 HUD 的 4° 引导圆内。截获距离为 500 ft 至 10 NM。
4. **`LR BST` — Long-Range Boresight（远距轴线瞄准）：** 沿雷达瞄准轴线搜索，HUD 显示 2.5° 引导圆；截获距离为 3,000 ft 至 40 NM。

---
title: Поддержка
sidebar: false
prev: false
next: false
---

# Поддержка

Здесь можно получить помощь, отправить отзыв, ознакомиться с политикой конфиденциальности и найти официальные каналы RedFlagSim.

## Связь и обратная связь

- Сообщайте об ошибках и предложениях через страницу [обратной связи](/ru/feedback).
- Email команды разработки: [support@redflagsim.com](mailto:support@redflagsim.com?subject=Red%20Flag%20Sim%20Support)
- Ответы на типовые вопросы находятся в разделе [FAQ](/ru/faq).

## Политики

- [Политика конфиденциальности](/ru/privacy-policy)

## Официальные каналы

- [Instagram](https://www.instagram.com/carrier_landing_hd/)
- [YouTube](https://www.youtube.com/@carrierlandinghd3129)

Актуальные ссылки на магазины и платформы размещены на [главной странице RedFlagSim](/ru/).

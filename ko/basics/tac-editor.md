...
Airbase는 공항과 주기 위치를 사용하며 플레이어 편대의 가장 안전한 기본값입니다. Airborne은 위도, 경도, 고도, 진방위와 보정 대기 속도를 사용합니다. Surface는 지상·수상·시설용입니다.

일반 항공기 Slot의 Surface 생성은 미구현이므로 Airbase 또는 Airborne을 사용하세요. 공중 생성 방위는 진북이 0이고 시계 방향으로 증가합니다. 공항에서는 주기 위치가 위치·자세와 초기 지상 상태를 결정합니다.
...
Flight 항로는 항법점으로 구성되며 출발, 집결, 요격, 표적, 복귀 또는 훈련 패턴을 나타낼 수 있습니다.

- 출발 공항 또는 공중 시작점에서 시작합니다.
- 항법점 사이를 명확히 띄웁니다.
- 전술 표적이나 공격 진입점을 의도적으로 표시합니다.
- 훈련 목적이 아니라면 저고도 항로가 지형을 통과하지 않게 합니다.
- 거리나 순항 속도를 바꾼 뒤 시간을 다시 확인합니다.

항법 시스템과 F-15C 화면은 임무 항로 데이터를 사용하므로 명확한 항법점은 조종석 사용성을 높입니다.
...
Bullseye는 전술 교신과 항전 표시의 공통 기준점입니다. 작전 구역, 사격장 중심, 알려진 지형 등 의미 있는 위치에 놓습니다.

지오펜스는 지도 오버레이와 경계로, 훈련 구역, 제한 공역, 위협 원, 표적 구역, 진입·이탈 통로, 사격·비행 금지 구역에 사용합니다. 규칙을 강제하지 않아도 전술 상황을 이해하기 쉽게 합니다.
...
비플레이어 요소는 Package/Flight 밖에 추가하여 SAM 진지, 지상군, 함선, 시설, 사격장 표적, 독립 항공기를 만듭니다. 항로 비행이나 편대 비행은 Package의 Flight를, 정적·전술 지도 물체는 비플레이어 요소를 사용하세요.
...
- Mission: 이름, 난도, 지형, 날씨·시간, 청·홍 공항, Bullseye, 제한과 전역 설정.
- Package: 이륙과 표적 도착 시간 기준을 가진 진영 단위 임무군.
- Flight: 호출부호, 진영, 공항, 역할, 항로, AI 행동과 항공기 Slot을 가진 편대.
- Slot: 항공기 한 대이며 플레이어 기체도 Slot입니다.
- Steerpoints: 항법과 시간 계획용 경유점.
- GeoFence: 전술 경계, 훈련·위협 구역, 기준 오버레이.
- 비플레이어 요소: Package에 속하지 않는 지상·수상·시설·독립 항공기.

임무 초안으로 저장됩니다. Go Fly를 누르면 World 장면을 불러오기 전에 실행용 임무 스냅샷으로 복제합니다.

### 임무 설정

환경과 출발 전 변경 가능 항목을 정합니다. 이름은 편집기와 목록에 표시됩니다. Scenery는 지형·공항 데이터 기준으로 Inspector에서 읽기 전용입니다. 날씨·시간은 World 시작 시 적용하고 청·홍 공항은 새 Flight/Slot의 기본값입니다. Bullseye는 공통 전술 기준이며 설정 권한은 무장·도장·날씨 변경 여부를 정합니다.

지형과 공항을 먼저 확인하세요. 편대 공항과 자동 생성 항법점 등 이후 기본값이 이에 의존합니다.

### 임무 제한

사용 가능한 항공기와 무장을 제한합니다. 기체 세대·시대 및 무장·유도 방식인 적외선, 반능동·능동·수동 레이더, TV 유도 등이 포함됩니다.

기총만 쓰는 근접전에서는 미사일을 제한하고, 가시거리 밖 훈련에서는 계획한 기체와 무장으로 제한하여 훈련 목표에 집중하세요.
...
1. 지형과 청·홍 기본 공항 확인.
2. 이름, 기술 수준, 날씨, 시간, Bullseye, 전역 설정 지정.
3. 청·홍 Package 생성.
4. Flight를 추가하고 역할 배정.
5. Slot을 추가하고 기종·생성·도장·연료·무장 선택.
6. 항법점으로 항로 작성.
7. 지오펜스와 비플레이어 요소 추가.
8. 제한과 플레이어 접근 조건 확인.
9. 저장 후 Go Fly로 시험.

작게 나눠 시험하세요. 플레이어 편대 하나로 정상 시작하는 임무가 검증 안 된 많은 유닛을 가진 임무보다 원인을 찾기 쉽습니다.

### 저장·불러오기와 Go Fly

저장·불러오기로 재사용할 초안을 보관합니다. 기종, 생성 방식, 항로 시간, 제한 등 큰 변경 뒤에는 시험해야 합니다.

Go Fly는 공통 시작 흐름을 사용합니다. World 로딩 전에 임무 데이터 존재, 플레이어 기체가 정확히 한 대인지, 모듈 접근 권한, 스냅샷 생성 가능 여부, 선택 기능·파생형·비기본 도장 팩 허용 여부를 확인합니다.

실패하면 플레이어 한 대, 공항 생성, 기본 무장·도장으로 단순화해 시험하고 조금씩 복잡성을 더하세요.
...
- 전술 구역 근처 Bullseye.
- 플레이어 Slot이 정확히 하나.
- 플레이어는 Airbase 또는 Airborne 생성.
- 각 Flight에 호출부호·진영·역할·항로가 있음.
- 무장·도장이 권한에 맞음.
- 지오펜스와 표적 구역이 명확함.
- 제한이 훈련 목표에 맞음.
- Go Fly로 최소 한 번 시험함.

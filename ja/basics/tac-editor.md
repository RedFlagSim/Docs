...
Airbase は飛行場と駐機場所を使い、プレイヤー編隊の安全な既定値です。Airborne は緯度、経度、高度、真方位、較正対気速度を使います。Surface は地上・水上・施設用です。

通常の航空機 Slot の Surface 出現は未実装です。Airbase または Airborne を使用してください。空中出現の方位は真北が 0 で時計回りに増加します。飛行場では駐機位置が位置・姿勢と初期地上状態を決めます。
...
航路は航法点で構成し、出発、集合、迎撃、目標、帰投、訓練パターンを表せます。

- 出発飛行場または空中開始点から始める。
- 航法点の間隔を明確にする。
- 戦術目標や攻撃開始点を意図的に指定する。
- 訓練目的でない限り低高度の航路を地形に通さない。
- 距離や巡航速度の変更後は時刻を再確認する。

航法システムと F-15C 表示は任務の航路を読むため、明確な航法点は操縦席での使いやすさを改善します。
...
Bullseye は戦術交信と航空電子表示に共通の基準点です。作戦区域、射場中央、既知の地形の近くなど意味のある場所に置きます。

ジオフェンスは地図の重ね合わせと境界で、訓練区画、制限空域、脅威円、目標区域、進入・離脱回廊、射撃禁止・飛行禁止区域に使えます。ゲーム規則を強制しなくても状況の理解を助けます。
...
非プレイヤー要素は Package/Flight の外に追加でき、SAM 陣地、地上部隊、艦船、施設、射場標的、独立した航空機に使います。航路飛行や編隊参加には Package の Flight、静的・戦術的な地図物体には非プレイヤー要素を使います。
...
- Mission：名前、難易度、地形、天候・時刻、青赤飛行場、Bullseye、制限、全体設定。
- Package：離陸と目標到達時刻の基準を持つ陣営単位の任務群。
- Flight：呼出符号、陣営、飛行場、役割、航路、AI 行動、機体 Slot を持つ編隊。
- Slot：1機分。プレイヤー機も Slot。
- Steerpoints：航法と時刻管理の経由点。
- GeoFence：戦術境界、訓練・脅威区域、参考表示。
- 非プレイヤー要素：Package に属さない地上・水上・施設・独立航空機。

これらは任務草稿に保存されます。Go Fly では World シーンを読む前に実行用任務スナップショットへ複製します。

### 任務設定

任務設定は環境と開始前に変更できる内容を決めます。名前はエディターと一覧に表示、Scenery は地形・飛行場データの基準で Inspector では読み取り専用です。天候・時刻は World 開始時に適用し、青赤飛行場は新規 Flight/Slot の既定値になります。Bullseye は共通の戦術基準で、設定権限は兵装・塗装・天候の変更可否を決めます。

地形と飛行場を先に確認してください。編隊の飛行場や生成される航法点など、後の既定値が依存します。

### 任務の制限

使用できる機体と兵装を制限します。機体の世代・年代、兵装や誘導方式（赤外線、セミアクティブ、アクティブ、パッシブ・レーダー、テレビ誘導）などがあります。

訓練目的に合わせ、機関砲のみの近接戦ではミサイルを制限し、視程外訓練では予定の機体・武器に限定します。
...
1. 地形と青赤の既定飛行場を確認。
2. 名前、技能レベル、天候、時刻、Bullseye、全体設定を指定。
3. 青赤 Package を作成。
4. Flight を追加し役割を指定。
5. Slot を追加し機種、出現方式、塗装、燃料、兵装を選択。
6. 航法点で航路を作成。
7. ジオフェンスと非プレイヤー要素を追加。
8. 制限とプレイヤー利用権限を確認。
9. 保存して Go Fly で試験。

少しずつ試してください。プレイヤー編隊1つで起動する任務は、未試験の多数のユニットを含む任務より原因を調べやすくなります。

### 保存・読込と Go Fly

保存・読込で再利用できる草稿を残します。機種、出現方式、航路時刻、制限などの大きな変更後は試験してください。

Go Fly は共通の起動処理を使います。World 読込前に任務データの存在、プレイヤー機が正確に1機あること、モジュール利用権、スナップショット作成、選択した機能・派生型・非標準塗装パックの許可を確認します。

失敗したらプレイヤー機1機、飛行場出現、既定兵装・塗装に単純化して試し、順に複雑さを戻します。
...
- Bullseye が戦術区域の近く。
- プレイヤー Slot は1つだけ。
- プレイヤーは Airbase または Airborne 出現。
- 各 Flight に呼出符号、陣営、役割、航路がある。
- 兵装・塗装が許可に合う。
- ジオフェンスと目標区域が見分けやすい。
- 制限が訓練目的に合う。
- Go Fly で1回以上試験済み。
